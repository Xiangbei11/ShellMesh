...
# exit()
total_surface_upper = np.append(surface_upper0[[0,1],:],surface_upper0[3:,:],axis=0)
...
total_surface_lower = np.append(surface_lower0[[0,1,],:],surface_lower0[3:,:],axis=0)
...
constrained_edges = np.append(constrained_edges, \
    np.array([[7,15],[15,23],[23,31],[31,55]\
        ]), axis = 0)

num_points_uu0 = 45
num_points_vv0 = 8
in_start = 55
for i in np.arange(in_start,in_start+num_points_uu0*num_points_vv0-40,num_points_vv0):
    constrained_edges = np.append(constrained_edges, np.array([i, i+num_points_vv0]).reshape(1,2), axis = 0)  
for i in np.arange(in_start+num_points_uu0*num_points_vv0-40,in_start+num_points_uu0*num_points_vv0-40-num_points_vv0+1,-1):
    constrained_edges = np.append(constrained_edges, np.array([i, i-1]).reshape(1,2), axis = 0)
for i in np.arange(in_start+num_points_uu0*num_points_vv0-40-num_points_vv0+1,40,-num_points_vv0):
    constrained_edges = np.append(constrained_edges, np.array([i, i-num_points_vv0]).reshape(1,2), axis = 0)
constrained_edges = np.append(constrained_edges, \
    np.array([[40,37],[37,35],[35,32],[32,24],[24,16],[16,8],[8,0]]), axis = 0)#
