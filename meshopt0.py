...
class meshopt(object):
    def __init__(self,vertexCoords,trilist,quadlist,w1,w2,w3,w4,itr,plot,fixedvert,dupvert=np.empty([0],dtype=np.int32)):
        '''
    ...
    def optimization(self):
        ...
        for i in range(len(itr)):
            if itr[i] == 0:
                ...
                self.trilist,self.quadlist = self.merging('preliminary_results/step'+str(i+1)+'_merging'+str(k+1)+'.png')
                k=k+1
            elif itr[i] == 2:
    ...
    def splitting(self,filename):
        ...
        meshopt = pymeshopt.Pymeshopt(vertexCoords,trilist,quadlist,self.w1,self.w2,self.w3,self.w4)
