...
for i in range(111,157):
    if i == 125 or i ==123:
        ...
        if i == 111:
            ...
            num_points_u0, num_points_v0  = 5, 3
        elif i == 114:
            num_points_u0, num_points_v0  = 7, 3
        elif i >= 154:
            num_points_u0, num_points_v0  = 2, 3#3, 2
        elif i >= 150 and i <= 156:
            num_points_u0, num_points_v0  = 3, 3
        elif i >= 140 and i <= 149:
            num_points_u0, num_points_v0  = 4, 3  
        elif i >= 132 and i <= 139:
            num_points_u0, num_points_v0  = 5, 3     
        elif i >= 123 and i <= 131:
            num_points_u0, num_points_v0  = 6, 3   
...
vd48 = vedo.Points((total_surface_upper[rib48_indices_updated,:]/scale), r=30, c = 'yellow', alpha = 0.8)
vd49 = vedo.Points((total_surface_upper[rib49_indices,:]/scale), r=30, c = 'brown', alpha = 0.8)
# vd_plotter0 = vedo.Plotter()
# vd_plotter0.show(vd4,vd5,vd00,vd01,vd48,vd49,'000', axes=1, viewup="z", interactive = True)#True
# exit()
...
# constrained_edges = np.append(constrained_edges, np.array([29, 30]).reshape(1,2), axis = 0)##
...
constrained_node_indices += rib48_indices_updated
...
points = np.append(total_surface_upper[rib48_indices_updated,:],total_surface_lower[rib48_indices_updated,:], axis = 0)
print('TTTT',len(points),points)
ids = [0,2]
constrained_edges = np.empty((0,2),dtype = int)
for i in range(len(rib48_indices_updated)-1):
    constrained_edges = np.append(constrained_edges, np.array([i, i+1]).reshape(1,2), axis = 0)
constrained_edges = np.append(constrained_edges, np.array([[i+1,2*len(rib48_indices_updated)-1]]).reshape(1,2), axis = 0)
for i in np.arange(2*len(rib48_indices_updated)-1,len(rib48_indices_updated),-1):
    constrained_edges = np.append(constrained_edges, np.array([i, i-1]).reshape(1,2), axis = 0)
constrained_edges = np.append(constrained_edges, np.array([[len(rib48_indices_updated),0]]).reshape(1,2), axis = 0)
...
ids = [1,2]
constrained_edges = np.empty((0,2),dtype = int)
for i in range(len(rib49_indices)-1):
    constrained_edges = np.append(constrained_edges, np.array([i, i+1]).reshape(1,2), axis = 0)
constrained_edges = np.append(constrained_edges, np.array([[i+1,2*len(rib49_indices)-1]]).reshape(1,2), axis = 0)
for i in np.arange(2*len(rib49_indices)-1,len(rib49_indices),-1):
    constrained_edges = np.append(constrained_edges, np.array([i, i-1]).reshape(1,2), axis = 0)
constrained_edges = np.append(constrained_edges, np.array([[len(rib49_indices),0]]).reshape(1,2), axis = 0)
A = dict(vertices=points[:,ids], segments=constrained_edges)
B = tr.triangulate(A,'p') 
# tr.compare(plt, A, B)         
# plt.show()#block=False
...
for memb in shell_mesh.members_dict.values():
    ...
    if memb.options['name'] == 'upper_surface':
        ...
        vd_points.append(vedo.Points((pts[:7]+pts[7:])/2, r=15, c='red',alpha = 0.7))
        vd_points.append(vedo.Points(pts, r=15, c='red',alpha = 0.7))
    elif memb.options['name'] == 'side0':
        pass
        #vd_points.append(vedo.Points(pts, r=15, c='green'))
        #vd_points.append(vedo.Points(pts[421,:].reshape(1,3), r=30, c='black'))
    elif memb.options['name'] == 'side1':
        pass
        #vd_points.append(vedo.Points(pts, r=15, c='black'))
    else:
        pass
        # color = list(vedo.colors.colors.values())[memb.options['id']]
        # vd_points.append(vedo.Points(pts, r=10, c=color,alpha = 0.7))
print('count',count,'num_u0_root',num_u0_root)
vd_plotter1 = vedo.Plotter()
vd_plotter1.show(vd_points,'222', viewup="z", interactive = True)
exit()
shell_mesh.optimizie_mesh()
shell_mesh.construct_whole_structure_optmesh('CAD_uCRM_shellmesh_51')
...
print('count',count)
shell_mesh.optimizie_mesh()
shell_mesh.construct_whole_structure_optmesh('CAD_uCRM_shellmesh_44')
vd_plotter1 = vedo.Plotter()
