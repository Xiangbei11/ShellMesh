...
shell_mesh.members_dict['side0'] = Member(  
    ...
    coordinates = points/scale,
    tri_connectivity = connectivity,
    constrained_node_indices = list(np.arange(len(side0_indices))) + list(np.arange(len(side0_indices),2*len(side0_indices))) + list(np.arange(2*len(side0_indices),3*len(side0_indices))),
    )  
k+=1
...
shell_mesh.members_dict['side1'] = Member(
    ...
    coordinates = points/scale,
    tri_connectivity = connectivity,
    constrained_node_indices = list(np.arange(len(side1_indices))) + list(np.arange(len(side1_indices),2*len(side1_indices))) + list(np.arange(2*len(side0_indices),3*len(side0_indices))),
    )  
...
for memb in shell_mesh.members_dict.values():
    ...
    if memb.options['name'] == 'lower_surface':
        pass
    else:
        vd_mesh.append(mesh)
    if memb.options['name'] == 'upper_surface':
        ...
        #print(len(pts),pts)
        vd_points.append(vedo.Points(pts, r=20, c='green'))
    else:
        vd_points.append(vedo.Points(pts, r=20, c='red',alpha = 0.7))
print('count',count)
shell_mesh.optimizie_mesh()
shell_mesh.construct_whole_structure_optmesh('CAD_uCRM_shellmesh_6_1')
vd_plotter1 = vedo.Plotter()
